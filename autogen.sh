...
(automake --version) >/dev/null 2>/dev/null || (echo "You need GNU automake 1.14 to install from sources (ftp://ftp.gnu.org/gnu/automake/)"; exit 1) || exit 1
...
if expr "1.14" \> "$automake_version" >/dev/null; then
  automake --version | head -n 1
  echo "Fatal error: automake version 1.14 or higher is required."
  exit 1
fi
