...
uint16 CUploadQueue::GetMaxSlots() const
{
        ...
        if (thePrefs::GetMaxUpload() == UNLIMITED) {
                float kBpsUp = theStats::GetMaxUploadRate() / 1024.0f;
                nMaxSlots = (uint16)(kBpsUp / kBpsUpPerClient) + 2;
		AddDebugLogLineN(logUploadSlots, CFormat(wxT("max upload rate: %f kB/s,    client slot: %f kB/s     ==> max slots: %d (actual slots: %d)")) 
		% kBpsUp % kBpsUpPerClient % nMaxSlots % m_uploadinglist.size());
        } else {
