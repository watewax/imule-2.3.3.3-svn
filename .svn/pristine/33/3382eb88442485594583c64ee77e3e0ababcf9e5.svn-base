...
function output_sig_image()
{
	...
	if ($values["running"])
		imagettftext($finalimg, FONTSIZE, 0, 26, 19, $fgcolor,
			FONTFILE, "iMule ".$values["muleversion"].
			" ".$text["runtimemsg"]." ".$values["psuptime"]);
	else
		imagettftext($finalimg, FONTSIZE, 0, 26, 19, $fgcolor,
			FONTFILE, "iMule ".$text["norunmsg"]);
	...
	if($values["kad"] == "2")
	{
		if ($values["running"] && $values["connected"] == "2")
			imagettftext($finalimg, FONTSIZE, 0, 25, 41, $fgcolor, FONTFILE,
				"iMule ".$text["connmsg"]." ".$text["kadonmsg"]);
	...
	} else if($values["kad"] == "1")
        {
                if ($values["running"] && $values["connected"] == "2")
                        imagettftext($finalimg, FONTSIZE, 0, 25, 41, $fgcolor, FONTFILE,
                                "iMule ".$text["connmsg"]." ".$text["kadfwmsg"]);
                else if ($values["running"] && $values["connected"] != "0" &&
                        $values["highlowid"] == "H")
                        imagettftext($finalimg, FONTSIZE, 0, 25, 41, $fgcolor, FONTFILE,
                ...
                if ($values["running"] && $values["connected"] == "2")
                        imagettftext($finalimg, FONTSIZE, 0, 25, 41, $fgcolor, FONTFILE,
                                "iMule ".$text["connmsg"]." ".$text["kadoffmsg"]);
                else if ($values["running"] && $values["connected"] != "0" &&
                        $values["highlowid"] == "H")
                        imagettftext($finalimg, FONTSIZE, 0, 25, 41, $fgcolor, FONTFILE,
...
else
{
	...
	if ($values["running"])
		echo $datacolstart."iMule ".$values["muleversion"]." ".
			$text["runtimemsg"]." ".$values["psuptime"].$datacolend;
	else
		echo $datacolstart."iMule ".$text["norunmsg"].$datacolend;
