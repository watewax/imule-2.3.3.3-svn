...
CWebserverGSocketFuncTable::CWebserverGSocketFuncTable() : m_lock(wiMUTEX_RECURSIVE)
{
...
bool CamulewebApp::GetTemplateDir(const wxString& templateName, wxString& templateDir)
{
        ...
        dir = CConfigDir::GetConfigDir(wxT("remote.conf")) + wxT("webserver");
        if (CheckDirForTemplate(dir, templateName)) {
                templateDir = dir;
...
wxString CamulewebApp::SetLocale(const wxString& language)
{
        ...
        if (!lang.IsEmpty()) {
                ...
                wxString dir;
                if (m_localTemplate) {
                        dir = JoinPaths(JoinPaths(JoinPaths(CConfigDir::GetConfigDir(), wxT("webserver")), m_TemplateName), wxT("locale"));
