...
const uint16 CPartFile::NO_PART = 0xFFFF ;
...
void CPartFile::CreatePartFile()
{
        ...
        m_taglist.push_back( CTag(TAG_PARTFILENAME, strPartName ) ) ;
...
// Now to flush the map into the list (Slugfiller)
        ...
        for ( ; it != gap_map.end(); ++it ) {
                ...
                if ((gap->start != (uint64)-1) && (gap->end != (uint64)-1) && gap->start <= gap->end && gap->start < GetFileSize()) {
