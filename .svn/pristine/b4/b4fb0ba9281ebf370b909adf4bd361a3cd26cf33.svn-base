...
uint32 GetTypeSize(uint8 type)
{
        ...
        case KB: size = 1<<10; break;
        case MB: size = 1<<20; break;
        case GB: size = 1<<30; break;
...
CPath CConfigDir::nomadFile ;
wxString CConfigDir::startDir  = wxT("");
wxString CConfigDir::configDir = wxT("");

wxString CConfigDir::GetConfigDir(const wxString & WXUNUSED(configFile) )
{
        if (configDir.IsEmpty()) {
                if (IsNomad())
                        configDir = startDir + wxFileName::GetPathSeparator() ;
                else
		  configDir = wxStandardPaths::Get().GetUserLocalDataDir() + wxFileName::GetPathSeparator();
        }
        return configDir ;
}

wxString CConfigDir::GetExecutableDir()
{
        if (startDir.IsEmpty()) {
                //startDir = wxFileName::GetCwd() ;
	  startDir = wxFileName(wxStandardPaths::Get().GetExecutablePath()).GetPath();
                nomadFile = CPath(JoinPaths(startDir, wxT("thisIsImuleConfigDir"))) ;
        }
        return startDir;
}

bool CConfigDir::IsNomad()
{
        if (startDir.IsEmpty()) {
	  startDir = wxFileName(wxStandardPaths::Get().GetExecutablePath()).GetPath();
                nomadFile = CPath(JoinPaths(startDir, wxT("thisIsImuleConfigDir"))) ;
        }
        return nomadFile.FileExists() ;
}

void CConfigDir::MakeNomad()
{
        if (!IsNomad()) {
                nomadFile.Touch();
        }
...
void CConfigDir::MakePerUserConfig()
{
        if (IsNomad()) {
                CPath::RemoveFile(nomadFile);
        }
}
...
#endif
        locale.AddCatalogLookupPathPrefix(JoinPaths(CConfigDir::GetExecutableDir(), wxT("locale"))); // nomad installation
