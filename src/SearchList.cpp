...
wxString CSearchList::StartNewSearch(uint32* searchID, SearchType type, const CSearchParams& params)
{
        ...
        if (type == KadSearch) {
                ...
                CPacket* searchPacket = new CPacket(*data, OP_EDONKEYPROT, OP_SEARCHREQUEST);
...
CSearchList::CMemFilePtr CSearchList::CreateSearchData(const CSearchParams& params, SearchType WXUNUSED(type), bool supports64bit, bool& packetUsing64bit)
{
...
#ifdef __DEBUG__
        AddDebugLogLineN( logSearch, CFormat(wxT("Search parsing result for \"%s\": %i"))
        ...
        AddDebugLogLineN(logSearch, mes);
        AddDebugLogLineN(logSearch, CFormat(wxT("Expression count: %i")) % _SearchExpr.m_aExpr.GetCount());
#endif

        parametercount += _SearchExpr.m_aExpr.GetCount();

#ifdef __DEBUG__
        AddDebugLogLineN(logSearch, CFormat(wxT("Parameters: %i")) % parametercount);
#endif
