...
void CFileDetailDialog::UpdateData(bool resetFilename)
{
        ...
        bufferS = CFormat(wxT("%" PRIu64 " bytes (%s)")) % m_file->GetFileSize() % CastItoXBytes(m_file->GetFileSize());
        CastChild(IDC_FSIZE,wxControl)->SetLabel(bufferS);
        CastChild(IDC_PFSTATUS,wxControl)->SetLabel(m_file->getPartfileStatus());
        bufferS = CFormat(wxT("%" PRIu16 " (%" PRIu64 ")")) % m_file->GetPartCount() % (uint64)m_file->GetHashCount();
        ...
        bufferS = CFormat(_("%.1lf%% done")) % m_file->GetPercentCompleted();
